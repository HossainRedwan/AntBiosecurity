...
def yolo_line_to_shape(self, class_index, x_center, y_center, w, h):
        ci = int(float(class_index))
        # Ensure classes list is long enough
        if ci >= len(self.classes):
            for i in range(len(self.classes), ci + 1):
        ...
        x_min = max(x_center - w / 2.0, 0.0)
        x_max = min(x_center + w / 2.0, 1.0)
        y_min = max(y_center - h / 2.0, 0.0)
        y_max = min(y_center + h / 2.0, 1.0)

        # Convert normalized coords to pixels (img_size = [height, width])
...
def parse_yolo_format(self):
        with open(self.file_path, 'r', encoding=ENCODE_METHOD, errors='ignore') as bnd_box_file:
