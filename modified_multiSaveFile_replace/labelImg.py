...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def _save_annotations_to_multi_dirs(self, base_save_dir, image_basename_no_ext):
        ...
        # Build shapes, same as save_labels()
        def format_shape(s):
            return dict(
                label=s.label,
                line_color=s.line_color.getRgb(),
        ...
        print(f'Image:{self.file_path} -> Annotations written under: {base_save_dir}')
        return True
    ...
    def _save_file(self, annotation_file_path):
        ...
        # If user selected a subfolder like XML_Only/TXT_Only/XML_and_TXT as Save Dir,
        # use its *parent* folder as the base.
        ...
        if self.default_save_dir and len(ustr(self.default_save_dir)):
            selected_dir = ustr(self.default_save_dir)
            norm_selected = os.path.normpath(selected_dir)
            leaf = os.path.basename(norm_selected)
            if leaf in {"XML_and_TXT", "XML_Only", "TXT_Only"}:
                base_save_dir = os.path.dirname(norm_selected)
            else:
                base_save_dir = norm_selected
            image_file_name = os.path.basename(self.file_path)
            basename_no_ext = os.path.splitext(image_file_name)[0]
        else:
            fallback_dir = os.path.dirname(self.file_path)
            base_save_dir = os.path.normpath(fallback_dir)
            image_file_name = os.path.basename(self.file_path)
            basename_no_ext = os.path.splitext(image_file_name)[0]
