...
Objective: Copies image data from a user-specified dataset folder to a "ToAnnotate" folder located in the same directory as the script. Organise dataset for auto annotation and YOLO model training.

Features:
    - Recursively searches all species subfolders for images in a user-specified dataset folder.
    - Creates or looks for a ToAnnotate folder next to your notebook/script.
    - Creates or looks for species folders with a nested <Species>_Images folder.
    - Renames files with timestamp for uniqueness.
    - Normalizes folder matching (ignores case, spaces, underscores, and dashes).
    - Skips images inside any folder named "not good images" (case-insensitive).
        ...
        for species_folder in self.dataset_folder.iterdir():
            ...
            for img_path in species_folder.rglob("*"):
                if img_path.is_file() and self.is_image_file(img_path):
                    # Skip if any parent folder is "not good images" (case-insensitive)
                    if any(self.normalize_name(p.name) == "notgoodimages" for p in img_path.parents):
                        continue
